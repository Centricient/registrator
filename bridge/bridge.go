...
func New(docker *dockerapi.Client, adapterUri string, config Config) (*Bridge, error) {
	...
	logger.Info("Using", uri.Scheme, "adapter:", uri)
	return &Bridge{
		docker:         docker,
		config:         config,
...
func (b *Bridge) Refresh() {
	...
	for containerId, services := range b.services {
		for _, service := range services {
			...
			if err != nil {
				logger.Error("refresh failed:", service.ID, err)
				continue
			}
			logger.Info("refreshed:", containerId[:12], service.ID)
		}
	}
}
...
func (b *Bridge) Sync(quiet bool) {
	...
	if err != nil && quiet {
		logger.Info("error listing containers, skipping sync")
		return
	} else if err != nil && !quiet {
		logger.Fatal(err)
	}

	logger.Infof("Syncing services on %d containers", len(containers))
	...
	for _, listing := range containers {
		...
		if services == nil {
			...
			for _, service := range services {
				...
				if err != nil {
					logger.Error("sync register failed:", service, err)
				}
			}
		}
	}
	...
	if b.config.Cleanup {
		...
		logger.Info("Listing non-exited containers")
		...
		if err != nil {
			logger.Error("error listing nonExitedContainers, skipping sync", err)
			return
		}
		for listingId, _ := range b.services {
			...
			if !found {
				logger.Infof("stale: Removing service %s because it does not exist", listingId)
				go b.RemoveOnExit(listingId)
			}
		}

		logger.Info("Cleaning up dangling services")
	...
	Outer:
		for _, extService := range extServices {
			...
			logger.Info("dangling:", extService.ID)
			err := b.registry.Deregister(extService)
			if err != nil {
				logger.Error("deregister failed:", extService.ID, err)
				continue
			}
			logger.Info(extService.ID, "removed")
		}
	}
}
...
func (b *Bridge) add(containerId string, quiet bool) {
	...
	if b.services[containerId] != nil {
		logger.Info("container, ", containerId[:12], ", already exists, ignoring")
	...
	if err != nil {
		logger.Error("unable to inspect container:", containerId[:12], err)
		return
	}
	...
	if len(ports) == 0 && !quiet {
		logger.Info("ignored:", container.ID[:12], "no published ports")
		return
	}
	...
	for key, port := range ports {
		if b.config.Internal != true && port.HostPort == "" {
			if !quiet {
				logger.Info("ignored:", container.ID[:12], "port", port.ExposedPort, "not published on host")
			}
			continue
		}
	...
	for _, port := range servicePorts {
		...
		if service == nil {
			if !quiet {
				logger.Info("ignored:", container.ID[:12], "service on port", port.ExposedPort)
			}
			continue
		}
		err := b.registry.Register(service)
		if err != nil {
			logger.Error("register failed:", service, err)
			continue
		}
		b.services[container.ID] = append(b.services[container.ID], service)
		logger.Info("added:", container.ID[:12], service.ID)
	}
}
...
func (b *Bridge) newService(port ServicePort, isgroup bool) *Service {
	...
	if b.config.UseIpFromLabel != "" {
		...
		if containerIp != "" {
			...
			logger.Info("using container IP " + service.IP + " from label '" +
				b.config.UseIpFromLabel + "'")
		} else {
			logger.Info("Label '" + b.config.UseIpFromLabel +
	...
	if networkMode != "" {
		if strings.HasPrefix(networkMode, "container:") {
			...
			logger.Info(service.Name + ": detected container NetworkMode, linked to: " + networkContainerId[:12])
			networkContainer, err := b.docker.InspectContainer(networkContainerId)
			if err != nil {
				logger.Error("unable to inspect network container:", networkContainerId[:12], err)
			} else {
				service.IP = networkContainer.NetworkSettings.IPAddress
				logger.Info(service.Name + ": using network container IP " + service.IP)
			}
		}
	}
	...
	if deregister {
		deregisterAll := func(services []*Service) {
			for _, service := range services {
				...
				if err != nil {
					logger.Error("deregister failed:", service.ID, err)
					continue
				}
				logger.Info("removed:", containerId[:12], service.ID)
			}
		}
...
func (b *Bridge) shouldRemove(containerId string) bool {
	...
	if _, ok := err.(*dockerapi.NoSuchContainer); ok {
		...
		logger.Errorf("registrator: container %v was removed, could not fetch exit code", containerId[:12])
		return true
	}

	switch {
	case err != nil:
		logger.Errorf("registrator: error fetching status for container %v on \"die\" event: %v\n", containerId[:12], err)
		return false
	case container.State.Running:
		logger.Infof("registrator: not removing container %v, still running", containerId[:12])
		return false
